...
use {
    super::utils::{
        get_peb, image_ordinal, image_snap_by_ordinal, Dll, Exe, BASE_RELOCATION_ENTRY, PE,
    },
    //super::cmd::Args,
    //clap::Parser,
    std::{
...
fn initialize_pe(buffer: Vec<u8>) -> Result<PE, String> {
    unsafe {
        ...
        let pe = PE {
            ...
            entry_tls_data: (*nt_header).OptionalHeader.DataDirectory
                [IMAGE_DIRECTORY_ENTRY_TLS.0 as usize],
            entry_import_data: (*nt_header).OptionalHeader.DataDirectory
                [IMAGE_DIRECTORY_ENTRY_IMPORT.0 as usize],
            entry_export_data: (*nt_header).OptionalHeader.DataDirectory
                [IMAGE_DIRECTORY_ENTRY_EXPORT.0 as usize],
            entry_exception: (*nt_header).OptionalHeader.DataDirectory
                [IMAGE_DIRECTORY_ENTRY_EXCEPTION.0 as usize],
            entry_basereloc_data: (*nt_header).OptionalHeader.DataDirectory
    ...
    unsafe {
        ...
        if pe.entry_exception.Size != 0 {
            let func_entries = std::slice::from_raw_parts(
                address.offset(pe.entry_exception.VirtualAddress as isize)
                    as *mut IMAGE_RUNTIME_FUNCTION_ENTRY,
                (pe.entry_exception.Size / size_of::<IMAGE_RUNTIME_FUNCTION_ENTRY>() as u32)
                    as usize,
            );
        ...
        if pe.entry_tls_data.Size != 0 {
            let img_tls_directory = address.offset(pe.entry_tls_data.VirtualAddress as isize)
                as *mut IMAGE_TLS_DIRECTORY64;
            let img_tls_callback =
                (*img_tls_directory).AddressOfCallBacks as *mut PIMAGE_TLS_CALLBACK;
        ...
        let entry_point =
            address.offset((*pe.nt_header).OptionalHeader.AddressOfEntryPoint as isize);
        if pe.is_dll {
        ...
        for descriptor in import_descriptors {
            ...
            loop {
                ...
                let func_address = if image_snap_by_ordinal((*original_thunk).u1.Ordinal) {
                    ...
                    let import_by_name = address.add((*original_thunk).u1.AddressOfData as usize)
...
fn realoc_image(pe: &mut PE, address: *mut c_void) -> Result<(), String> {
    unsafe {
        ...
        let mut base_relocation = address.offset(pe.entry_basereloc_data.VirtualAddress as isize)
                ...
                match entry_type as u32 {
                    ...
                    IMAGE_REL_BASED_HIGHLOW => {
                        *(target_address as *mut u32) =
                            (*(target_address as *mut u32)).wrapping_add(offset as u32)
                    }
                    IMAGE_REL_BASED_HIGH => {
                        *(target_address as *mut u16) = (*(target_address as *mut u16) as u32)
                            .wrapping_add((offset as u32 >> 16) & 0xFFFF)
                            as u16
                    }
                    IMAGE_REL_BASED_LOW => {
                        *(target_address as *mut u16) = (*(target_address as *mut u16) as u32)
...
fn export_function_address(
    ...
    unsafe {
        let img_export_directory =
            (address + pe.entry_export_data.VirtualAddress as usize) as *mut IMAGE_EXPORT_DIRECTORY;
        let names = (address + (*img_export_directory).AddressOfNames as usize) as *const u32;
        let addresses =
            (address + (*img_export_directory).AddressOfFunctions as usize) as *const u32;
        let ordinals =
            (address + (*img_export_directory).AddressOfNameOrdinals as usize) as *const u16;

        for i in 0..(*img_export_directory).NumberOfFunctions as isize {
            let name_offset = *names.offset(i) as usize;
            let function_name =
                std::ffi::CStr::from_ptr((address + name_offset) as *const i8).to_string_lossy();
