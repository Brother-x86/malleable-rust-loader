...
impl ComplexResource {
    fn new(name: &str) -> Self {
        println!("{}{}", obfstr!("Initialisation de ComplexResource : "), name);
        ComplexResource {
            name: name.to_string(),
        }
    }
    ...
    fn perform_task(&self) {
        println!("{}{}", obfstr!("Exécution de la tâche sur la ressource : "), self.name);
    }
}
...
fn initialize_resource() {
    INIT.call_once(|| {
        let resource = ComplexResource::new(obfstr!("Resource1"));
        unsafe {
            RESOURCE = Some(Arc::new(Mutex::new(resource)));
        }
        println!("{}", obfstr!("Ressource initialisée avec succès!"));
    });
}
