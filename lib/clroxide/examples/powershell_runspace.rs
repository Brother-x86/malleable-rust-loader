...
unsafe fn runspace_execute(command: String) -> Result<String, String> {
    ...
    let mscorlib = (*app_domain).load_library(obfstr!("mscorlib"))?;
    ...
    let assembly_type = (*mscorlib).get_type(obfstr!("System.Reflection.Assembly"))?;
    let assembly_load_with_partial_name_fn = (*assembly_type).get_method_with_signature(
        obfstr!("System.Reflection.Assembly LoadWithPartialName(System.String)"),
    )?;
    let automation_variant = (*assembly_load_with_partial_name_fn).invoke(
        wrap_method_arguments(vec![wrap_string_in_variant(obfstr!("System.Management.Automation"))])?,
        None,
    )?;
    let automation =
    ...
    // Get types
    let psobject_type = (*automation).get_type(obfstr!("System.Management.Automation.PSObject"))?;
    let runspace_factory_type =
        (*automation).get_type(obfstr!("System.Management.Automation.Runspaces.RunspaceFactory"))?;
    let runspace_pipeline_commands_type =
        (*automation).get_type(obfstr!("System.Management.Automation.Runspaces.CommandCollection"))?;
    let runspace_pipeline_reader_type = (*automation).get_type(
        obfstr!("System.Management.Automation.Runspaces.PipelineReader`1[System.Management.Automation.PSObject]")
    )?;
    let runspace_pipeline_type =
        (*automation).get_type(obfstr!("System.Management.Automation.Runspaces.Pipeline"))?;
    let runspace_type =
        (*automation).get_type(obfstr!("System.Management.Automation.Runspaces.Runspace"))?;

    // Get functions
    let commands_addscript_fn = (*runspace_pipeline_commands_type)
        .get_method_with_signature(obfstr!("Void AddScript(System.String)"))?;
    let pipeline_create_fn = (*runspace_type).get_method_with_signature(
        obfstr!("System.Management.Automation.Runspaces.Pipeline CreatePipeline()"),
    )?;
    let pipeline_getoutput_fn = (*runspace_pipeline_type).get_method_with_signature(
        obfstr!("System.Management.Automation.Runspaces.PipelineReader`1[System.Management.Automation.PSObject] get_Output()")
    )?;
    let pipeline_invoke_async_fn =
        (*runspace_pipeline_type).get_method_with_signature(obfstr!("Void InvokeAsync()"))?;
    let pipeline_reader_read_fn = (*runspace_pipeline_reader_type)
        .get_method_with_signature(obfstr!("System.Management.Automation.PSObject Read()"))?;
    let psobject_tostring_fn =
        (*psobject_type).get_method_with_signature(obfstr!("System.String ToString()"))?;
    let runspace_create_fn = (*runspace_factory_type).get_method_with_signature(
        obfstr!("System.Management.Automation.Runspaces.Runspace CreateRunspace()"),
    )?;
    let runspace_dispose_fn = (*runspace_type).get_method(obfstr!("Dispose"))?;
    let runspace_open_fn = (*runspace_type).get_method(obfstr!("Open"))?;
    ...
    let pipeline_commands_property = (*runspace_pipeline_type).get_property(obfstr!("Commands"))?;
    ...
    (*commands_addscript_fn).invoke(
        wrap_method_arguments(vec![wrap_string_in_variant(
            format!("{}{}", command, obfstr!(" | Out-String")).as_str(),
        )])?,
        Some(commands),
    )?;
...
fn main() {
    println!("{}", obfstr!("Press enter to execute..."));
    let mut input = String::new();
    std::io::stdin().read_line(&mut input).unwrap();

    let result = unsafe { runspace_execute(obfstr!("Get-Process").to_string()) };
    match result {
        Ok(out) => println!("{}{}", obfstr!("Output:\n"), out),
        Err(err) => println!("{}{}", obfstr!("Error: "), err),
    }

    println!("{}", obfstr!("Done! Press enter to exit."));
    std::io::stdin().read_line(&mut input).unwrap();
}
